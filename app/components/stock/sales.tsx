...
const RecentStockSales = () => {
    ...
    useEffect(()=> {
      const fetchStock = async () => {
        try {
          const response = await fetch(`/api/stock/low_stock?search=${search}&startDate=${startDate}&category=${category}&batch=${batch}&supplier=${supplier}`);
      ...
      fetchStock();
  }, [search, category, startDate, supplier, batch]);
  ...
  return (
    ...
    <div className="pb-4 flex justify-between items-center">
      <h4 className="text-orange-800 ">Low stock</h4>
      <div>
        <div  
      ...
      <div className="pb-4 max-h-[40vh] overflow-hidden overflow-y-visible">
         <table className="w-full border-collapse">
          <thead className="text-sm text-neutral-600 bg-neutral-950 rounded-t ">
            <tr className="px-2 text-left">
              ...
              <th className="text-violet-900">{stock.length}</th>
            </tr>
          </thead>
          <tbody>
...
export default RecentStockSales;