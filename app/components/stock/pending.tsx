...
interface ListProps {
  onConfirm: (purchaseCode: string) => void;
}
...
const NextStockSales = ({onConfirm}:ListProps) => {
      ...
      useEffect(()=> {
        const fetchStock = async () => {
          try {
            const response = await fetch(`/api/stock/pending?startDate=${startDate}&supplier=${supplier}`);
  ...
  return (
    <div className="p-5 bg-neutral-900 rounded-lg h-max">
      ...
      <div className="pb-4 flex justify-between items-center">
        <h4 className="text-yellow-800 ">Pending stock</h4>
        <div>
          <div  
            ...
            >
             ...
             <form method="post" className="py-3 space-y-2">
              <div className="relative w-full">
                <input type="text" placeholder="By purchase code" onChange={handleSearch} className="bg-black rounded-md outline-0 border border-neutral-700 py-2 px-4 text-neutral-500 w-full focus:border-emerald-700" />
              </div>
              <div className="relative w-full">
                <input type="text" placeholder="By supplier" onChange={handleSupplier} className="bg-black rounded-md outline-0 border border-neutral-700 py-2 px-4 text-neutral-500 w-full focus:border-emerald-700" />
              </div>
              <div className="relative w-full">
           ...
           {currentStock.length > 0 && (
              currentStock.map((item, i) => (
                <tr key={i} className="text-sm text-neutral-500 border-b border-neutral-800">
                  ...
                  <td className="py-2 px-3 text-orange-900">{formatNumber(item.product.length, 0)}</td>
                  <td className="p-2">{item.supplier}</td>
                  <td className="p-2">{formatDate(item.created_at)}</td>
                  <td className="p-2"><button onClick={() => onConfirm(item.purchase_code)} className="border border-emerald-800 text-emerald-800 py-1 px-2 rounded-md text-xs cursor-pointer">Confirm</button></td>
                </tr>
              ))
            )}
          </tbody>
