...
interface ListProps {
  onPurchase: (purchaseCode: string) => void;
}

const OutOfStock = ({onPurchase}: ListProps) => {
    ...
    useEffect(()=> {
      const fetchStock = async () => {
        try {
          const response = await fetch(`/api/stock/out_of_stock?startDate=${startDate}&category=${category}&endDate=${endDate}&supplier=${supplier}`);
  ...
  return (
    <div className="p-5 bg-neutral-900 rounded-lg h-max">
      ...
      <div className="pb-4 flex justify-between items-center">
        <h4 className="text-red-800 ">Out Of stock</h4>
        <div>
        <div  
         ...
         <table className="w-full border-collapse">
          <thead className="text-sm text-neutral-600 bg-black rounded-t ">
            <tr className="px-2 text-left">
              ...
              <th className="p-2 font-normal">Expiry date</th>
          ...
          <tbody className=" max-h-[2vh] overflow-hidden overflow-y-visible">
           {currentStock.length > 0 && (
              currentStock.map((item, i) => (
                <tr key={i} className="text-sm text-orange-800 border-b bg-neutral-90 border-neutral-800">
                  <td className="p-2">{item.name}</td>
                  <td className="py-2">{item.supplier}</td>
                  <td className="p-2">{item.category}</td>
                  <td className="p-2">{formatDate(item.created_at)}</td>
                  <td className="p-2"><button onClick={() => onPurchase(item.id)} className="border border-teal-800 text-teal-800 py-1 px-2 rounded-md text-xs cursor-pointer">Purchase</button></td>
                </tr>
              ))
            )}
          </tbody>
